        ...
        for segment in data["results"]["speaker_labels"]["segments"]:
            ...
            if len(segment["items"]) > 0:
                ...
                for word in segment["items"]:
                    ...
                    word_result = list(
                        filter(lambda x: x["start_time"] == word["start_time"] and x["end_time"] == word["end_time"],
                               pronunciations))
                    try:
            ...
            if len(channel["items"]) > 0:
                ...
                for word in channel["items"]:
                    ...
                    if word["type"] == "pronunciation":
                        ...
                        word_result = list(filter(
                            lambda x: x["start_time"] == word["start_time"] and x["end_time"] == word["end_time"],
                            pronunciations))
                        try:
                            result = sorted(word_result[-1]["alternatives"], key=lambda x: x["confidence"])[-1]
                            confidence = float(result["confidence"])
...
def load_transcribe_job_status(cli_args):
    ...
    try:
        ...
        job_status = transcribe_client.get_call_analytics_job(CallAnalyticsJobName=cli_args.inputJob)[
            "CallAnalyticsJob"]
...
def generate_document():
    ...
    source_group.add_argument('--inputFile', metavar='filename', type=str,
                              help='File containing Transcribe JSON output')
    ...
    cli_parser.add_argument('--sentiment', choices=['on', 'off'], default='off',
                            help='Enables sentiment analysis on each conversational turn via Amazon Comprehend')
    cli_parser.add_argument('--confidence', choices=['on', 'off'], default='off',
                            help='Displays information on word confidence scores throughout the transcript')
